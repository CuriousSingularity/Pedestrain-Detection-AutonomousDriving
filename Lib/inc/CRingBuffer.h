 ...
 * context of non-commercial (unless permitted by h_da) and official h_da projects, is hereby
 * granted for enrolled students of h_da, provided that the above copyright notice, this paragraph
 * and the following paragraph appear in all copies, modifications, and distributions. Contact
 * Prof.Dr.-Ing. Peter Fromm, peter.fromm@h-da.de, Birkenweg 8 64295 Darmstadt - GERMANY for
 * commercial requests.
 *
 * \warning This software is a PROTOTYPE version and is not designed or intended for use in
 * production, especially not for safety-critical applications! The user represents and warrants
 * that it will NOT use or redistribute the Software for such purposes. This prototype is for
 * research purposes only. This software is provided "AS IS," without a warranty of any kind.
 */
...
class CRingBuffer {
  ...
  public:
     ...
     * Basic atomic write operation. After one element write, if write was possible, it is assumed
     * as completed. Returns error in case ringbuffer is full
     */
...
global::RC_t CRingBuffer<DATA, SIZE>::writeData(DATA const& data, fp_customCopy_t customFunction) {
    ...
    this->m_writeindex =
        (this->m_writeindex + 1) % SIZE;  // Move write index to next buffer's position
