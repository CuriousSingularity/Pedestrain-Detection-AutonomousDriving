...
struct ApplicationStatistics {
    ...
    constexpr double getAverageProcessingTime() const noexcept {
        return totalFramesProcessed > 0
                   ? static_cast<double>(totalProcessingTime.count()) / totalFramesProcessed
                   : 0.0;
    }
    ...
    std::string toString() const {
        return std::format("Frames: {}, Detections: {}, Commands: {}, Errors: {}, "
                           "Avg Processing: {:.2f}ms, Uptime: {}ms",
                           totalFramesProcessed, totalDetections, commandsExecuted, errorsHandled,
                           getAverageProcessingTime(), getUptime().count());
    }
};
...
struct ModernConfiguration {
    ...
    constexpr bool isValid() const noexcept
        requires requires(T t) { t.isValid(); }
    {
        return frameInterval.count() > 0 && maxDetectionResults > 0 && detectionConfig.isValid();
    }
};
...
class ModernCpp20Application : public IObserver {
  ...
  public:
     ...
     */
    explicit ModernCpp20Application(
        const ModernConfiguration<IDetectionAlgorithm::DetectionConfig>& config = {})
        : m_config{config} {
        initializeServices();
    }
    ...
    async::Task<global::RC_t> startAsync() {
        ...
        if (m_asyncDetectionService) {
            auto frameSource = [this]() -> cv::Mat { return getNextFrame(); };

            // Start frame processing coroutine
            auto processingGenerator =
                m_asyncDetectionService->processFrameStream(frameSource, m_config.frameInterval);
    ...
    processFramesRange(std::span<const cv::Mat> frames) {
        ...
        for (const auto& frame : frames | views::take(m_config.maxDetectionResults)) {
            ...
            auto filteredResults =
                results | confidence_filter(0.5f) |
    ...
    [[nodiscard]] std::string getFormattedStatistics() const {
        ...
        return std::format("Application Statistics:\n"
                           "  Frames Processed: {:L}\n"
                           "  Total Detections: {:L}\n"
                           "  Commands Executed: {:L}\n"
                           "  Errors Handled: {:L}\n"
                           ...
                           "  Uptime: {}",
                           stats.totalFramesProcessed, stats.totalDetections,
                           stats.commandsExecuted, stats.errorsHandled,
                           stats.getAverageProcessingTime(), stats.getUptime());
    }
     ...
     */
    void requestShutdown() noexcept { m_shutdownRequested.store(true); }

    /**
     * @brief Get configuration
     * @return const auto& Configuration reference
     */
    [[nodiscard]] const auto& getConfiguration() const noexcept { return m_config; }
  ...
  private:
    ...
    void initializeServices() {
        ...
        m_asyncDetectionService =
            std::make_unique<async::AsyncDetectionService>(m_detectionAlgorithm);
     ...
     */
    async::Task<void>
    processDetectionResults(std::vector<IDetectionAlgorithm::DetectionResult> results) {
    ...
    void handleDetectionEvent(const DetectionEventData& event) {
        ...
        auto highConfidenceDetections =
            event.detections | confidence_filter(0.7f) | std::ranges::to<std::vector>();
        ...
        if (!highConfidenceDetections.empty()) {
            ...
            auto logMessage = std::format("High confidence detections: {}, avg confidence: {:.2f}",
